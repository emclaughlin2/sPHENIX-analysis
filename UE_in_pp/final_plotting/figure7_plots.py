...
ROOT.gROOT.LoadMacro("sPhenixStyle.C");
...
# ============================================================
FNAME = ("plot_result_hist_output_efrac_bkg_cut_run28_iter_3_1000toys.root")
...
_exl     = [1.0,    1.0,    1.0,    2.0,    2.5,    2.5,    5.0,    5.0   ]
_exh     = [1.0,    1.0,    1.0,    2.0,    2.5,    2.5,    5.0,    5.0   ]
...
graph.SetFillColorAlpha(SPH_FILL_COLOR, 0.60)
...
#hframe.GetYaxis().SetTitle("<#deltaN_{ch}/#delta#eta#delta#phi>")
...
hframe.GetYaxis().SetTitleOffset(100.55)
hframe.GetYaxis().SetTitleOffset(100.55)
...
unfold_hist.Draw("E1 SAME")  # stat error bars on top
...
right_ax.SetLineColor(SPH_COLOR)
right_ax.SetTitleColor(SPH_COLOR)
right_ax.SetLabelColor(SPH_COLOR)
right_ax.Draw()

left_ax = ROOT.TGaxis(X_MIN, SPH_YMIN, X_MIN, SPH_YMAX,STAR_YMIN, STAR_YMAX, 510, "")
left_ax.SetTitle("<#deltaN_{ch}/#delta#eta#delta#phi>")
left_ax.SetTitleOffset(1.55)
left_ax.SetTitleSize(0.038)
left_ax.SetLabelFont(42)
left_ax.SetTitleFont(42)
left_ax.SetLabelSize(0.040)
left_ax.SetLineColor(STAR_COLOR)
left_ax.SetTitleColor(STAR_COLOR)
left_ax.SetLabelColor(STAR_COLOR)
left_ax.Draw()
...
leg = ROOT.TLegend(0.15, 0.15, 0.5, 0.28)
leg.SetBorderSize(0)
leg.SetFillStyle(0)
...
leg.AddEntry(star_sys,     "STAR (PRD 101 052004) <#kern[0.01]{#deltaN_{ch}/#delta#eta#delta#phi> #kern[0.01]{p_{T}^{ch} > 0.5 GeV}}",            "fpl")
leg.Draw()
...
pad2.Update()
c.Update()
c.SaveAs("sphenix_star_ue_comparison.pdf")
c.SaveAs("sphenix_star_ue_comparison.png")
print("Saved: sphenix_star_ue_comparison.pdf  and  .png")
