...
namespace
{
  ...
  //        delta_i(pT_j) = (b_i - b_nom) * (pT_j - 33)
  //      where 33 GeV is the chosen pivot, so the band has zero width at 33
  //      and the intercept (scale) is held fixed = nominal intercept.
                                    ...
                                    const std::string &tag)
  {
    ...
    for (size_t i = 1; i < h_vars.size(); ++i)
    {
      ...
      for (int b = 0; b < n; ++b)
      {
        // slope-only shift: pivot at 33 so the intercept (scale) is fixed
...
void figure6_plots(
    ...
    const char *output_name        = "h_jet_pt_bin_unc_figure.pdf",
    double      ptlo               = 21.,
    double      pthi               = 63.)
{
  ...
  g_dijet_slope->SetMarkerStyle(20);
  g_efrac_slope->SetMarkerColor(kAzure + 2);
  g_efrac_slope->SetMarkerStyle(33);
  g_efrac_slope->SetMarkerSize(1.4);
  ...
  leg->AddEntry(f_dijet_nom,Form("f_{ex. dijet}(p_{T}) = %.3f #pm %.3f + (%.4f #pm %.4f) p_{T}",a_dnom, a_dnom_err, b_dnom, b_dnom_err),"l");
  leg->AddEntry(f_efrac_nom,Form("f_{inc. jet}(p_{T}) = %.3f #pm %.3f + (%.4f #pm %.4f) p_{T}",a_enom, a_enom_err, b_enom, b_enom_err),"l");
