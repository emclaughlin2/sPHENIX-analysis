...
void draw_result_with_syst(std::vector<TH1D*> truth, std::vector<TH1D*> unfold, int dijet, const char* output_name = nullptr)
{
    ...
    leg->AddEntry(truth[0], "Pythia8", "pl");
    leg->AddEntry(truth[1], "Herwig7", "pl");
    ...
    for (int b = 1; b <= nbins; b++) {
        ...
        if (nom > 0) {
            ratio_nominal->SetBinContent(b, pyt / nom);
            ratio_nominal->SetBinError(b, pyt * stat / (nom * nom));
        }
        if (nom > 0) {
            ratio_herwig->SetBinContent(b, her / nom);
            ratio_herwig->SetBinError(b, her * stat / (nom * nom));
        }
    ...
    ratio_nominal->GetYaxis()->SetTitle("MC / Data");
    ratio_nominal->GetYaxis()->SetNdivisions(404);
    ratio_nominal->GetYaxis()->SetRangeUser(0.8, 1.5);
    ...
    ratio_nominal->SetMarkerStyle(21);
    ratio_herwig->SetMarkerStyle(22);
    ...
    TFile* f_hist_out;
    if (dijet) {
        f_hist_out = new TFile("plot_result_hist_output_dijet_bkg_cut_run28_iter_3_1000toys.root","RECREATE");
    } else {
        f_hist_out = new TFile("plot_result_hist_output_efrac_bkg_cut_run28_iter_3_1000toys.root","RECREATE");
    }
    f_hist_out->cd();
    unfold[0]->Write();
    g_syst->Write();
...
void figure5_plots(int dijet = 1) {
    ...
    if (dijet) {
        f = TFile::Open("output_mbd_correct_unfolded_data_8calibetbin_dijet_bkg_cut_run28_iter_3_1000toys.root");
        fherwig = TFile::Open("output_mbd_correct_herwig_unfolded_data_8calibetbin_dijet_bkg_cut_run28_iter_3_1000toys.root");
    } else {
        f = TFile::Open("output_mbd_correct_unfolded_data_8calibetbin_efrac_bkg_cut_run28_iter_3_1000toys.root");
        fherwig = TFile::Open("output_mbd_correct_herwig_unfolded_data_8calibetbin_efrac_bkg_cut_run28_iter_3_1000toys.root");
    }
    ...
    if (dijet) {
        draw_profile(h_truth, h_unfold, true, syst_tags, (int)h_unfold.size(), dijet, "h_profile_run28_w_calo_syst_8calibetbin_dijet_bkg_cut_figure.pdf");
        draw_result_with_syst(h_truth, h_unfold, dijet, "h_result_run28_w_calo_syst_8calibetbin_dijet_bkg_cut_syst_band_figure.pdf");
    } else {
        draw_profile(h_truth, h_unfold, true, syst_tags, (int)h_unfold.size(), dijet, "h_profile_run28_w_calo_syst_8calibetbin_efrac_bkg_cut_figure.pdf");
        draw_result_with_syst(h_truth, h_unfold, dijet, "h_result_run28_w_calo_syst_8calibetbin_efrac_bkg_cut_syst_band_figure.pdf");
    }
}
