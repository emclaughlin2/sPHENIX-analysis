...
void Fun4All_InclusiveJet(int nEvents = 100, int seg = 0, int isSim = 0, const char *filelistcalo = "dst_calo_cluster.list", const char *type = "run22_jet10", const char *filelistjet = "dst_jet.list")
{
  ...
  if (!isSim) {
    if (!strcmp(type,"1.5mrad")) {
      outfilename = "/sphenix/tg/tg01/jets/egm2153/UEinppOutput/topores_output1.5mrad_" + to_string(runnumber) + "_" + to_string(segment) + ".root";
      //outfilename = "/sphenix/user/egm2153/calib_study/analysis/UE_in_pp/output_" + to_string(runnumber) + "_" + to_string(segment) + ".root";
    } else {
      outfilename = "/sphenix/tg/tg01/jets/egm2153/UEinppOutput/topores_output_" + to_string(runnumber) + "_" + to_string(segment) + ".root";
      //outfilename = "/sphenix/user/egm2153/calib_study/JetValidation/analysis/output_" + to_string(runnumber) + "_" + to_string(segment) + ".root";
    }
  } else {
    if (!strcmp(type,"run22_jet10")) {
      outfilename = "/sphenix/tg/tg01/jets/egm2153/UEinppOutput/sim_run22_jet10_topores_output_" + to_string(seg) + ".root";
    } else if (!strcmp(type,"run22_jet30")) {
      outfilename = "/sphenix/tg/tg01/jets/egm2153/UEinppOutput/sim_run22_jet30_topores_output_" + to_string(seg) + ".root";
    } else if (!strcmp(type,"run22_mb")) { 
      outfilename = "/sphenix/tg/tg01/jets/egm2153/UEinppOutput/sim_run22_mb_topores_output_" + to_string(seg) + ".root";
    } else if (!strcmp(type,"run21_jet10")) {
      outfilename = "/sphenix/tg/tg01/jets/egm2153/UEinppOutput/sim_run21_jet10_topores_output_" + to_string(seg) + ".root";
    } else if (!strcmp(type,"run21_jet30")) {
      outfilename = "/sphenix/tg/tg01/jets/egm2153/UEinppOutput/sim_run21_jet30_topores_output_" + to_string(seg) + ".root";
    } else if (!strcmp(type,"run21_mb")) { 
      outfilename = "/sphenix/tg/tg01/jets/egm2153/UEinppOutput/sim_run21_mb_topores_output_" + to_string(seg) + ".root";
    }
     else {
      std::cout << "unknown simulation dst type" << std::endl;
    }
  ...
  */
  ...
  RawClusterBuilderTopo* ClusterBuilder = new RawClusterBuilderTopo("EMcalRawClusterBuilderTopo");
  ClusterBuilder->Verbosity(verbosity);
  ClusterBuilder->set_nodename("TOPOCLUSTER_EMCAL");
  ClusterBuilder->set_enable_HCal(false);
  ClusterBuilder->set_enable_EMCal(true);
  ClusterBuilder->set_noise(0.01, 0.03, 0.03);
