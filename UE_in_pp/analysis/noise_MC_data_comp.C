...
void noise_MC_data_comp(string outfilename = "dijet_calo_analysis_run21_jet10_topocluster.root", bool sim = false, bool clusters = true, bool emcal_clusters = false, const char* type = "1.5mrad") {
	...
	float ptbins[] = {16,18,20,22,24,27,30,35,40,50,60,80};
	...
	TH1F* h_et_spectra = new TH1F("h_et_spectra","",netbins, etbins);
 	...
 	string infilename;
	if (!strcmp(type,"MC")) {
 		infilename = "sim_run21_jet10_3sigma_output_*.root";
 	} else if (!strcmp(type,"1.5mrad")) {
 		infilename = "output1.5mrad_ana468_*_0.root";
 	} else if (!strcmp(type,"0mrad")) {
 		infilename = "output_0mrad_ana468_*_0.root";
 	} else {
    ...
    //for (Long64_t entry = 0; entry < 1000000; ++entry) {
    	...
    	// require jet trigger in data
  		...
  		// create leading and subleading jet vectors
  		TVector3 lead, sub;
  			...
  			if (!clusters) { // using towers to find total energy in towards, transverse and away regions 
	  			for (int i = 0; i < emcaln; i++) {
	  				...
	  				if (fabs(dphi) < M_PI/3.0) { // towards region 
						...
						h_et_spectra->Fill(emcale[i]/cosh(emcaleta[i]));
