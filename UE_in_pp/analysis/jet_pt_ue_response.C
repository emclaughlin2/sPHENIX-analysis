...
bool isInRange(float truthJetPt, int trig);
...
void jet_pt_unfolding(string infilename = "sim_run22_jet10_output_*.root", int trig = 10, string filename = "jet_pt_ue_response.root") {
    ...
    if (trig == 30) {
        weight = Jet30GeV_cross_section/Jet10GeV_cross_section;
    }
    ...
    //for (Long64_t entry = 0; entry < 2000; ++entry) {
        ...
        */
        ...
        if ((nJet >= 2 && lead.Pt() > leadptmin && sub.Pt() > subptmin && fabs(lead.DeltaPhi(sub)) > 2.75) || (nTruthJet >= 2 && truthlead.Pt() > leadptmin && truthsub.Pt() > subptmin && fabs(truthlead.DeltaPhi(truthsub)) > 2.75)) {
            ...
            h_pass_deltaphi->Fill(lead.DeltaPhi(sub), weight);
            h_pass_xj->Fill(sub.Pt()/lead.Pt(), weight);
            h_pass_spectra->Fill(lead.Pt(), weight);
            h_pass_spectra->Fill(sub.Pt(), weight);
            h_pass_aj_ptavg->Fill((lead.Pt()+sub.Pt())/2.0, (lead.Pt()-sub.Pt())/(lead.Pt()+sub.Pt()), weight);
            
            h_pass_truth_deltaphi->Fill(truthlead.DeltaPhi(truthsub), weight);
            h_pass_truth_xj->Fill(truthsub.Pt()/truthlead.Pt(), weight);
            h_pass_truth_spectra->Fill(truthlead.Pt(), weight);
            h_pass_truth_spectra->Fill(truthsub.Pt(), weight);
            h_pass_truth_aj_ptavg->Fill((truthlead.Pt()+truthsub.Pt())/2.0, (truthlead.Pt()-truthsub.Pt())/(truthlead.Pt()+truthsub.Pt()), weight);

            if ((nJet >= 2 && lead.Pt() > leadptmin && sub.Pt() > subptmin && fabs(lead.DeltaPhi(sub)) > 2.75) && (nTruthJet >= 2 && truthlead.Pt() > leadptmin && truthsub.Pt() > subptmin && fabs(truthlead.DeltaPhi(truthsub)) > 2.75)) {
                if (truthlead.DeltaR(lead) < dRMax && truthsub.DeltaR(sub) < dRMax) { // should this match be both the leading and subleading? 
                    // MATCH 
                    hMeasPT->Fill(lead.Pt(), weight);
                    hTruthPT->Fill(truthlead.Pt(), weight);
                    resp_full->Fill(lead.Pt(),truthlead.Pt(), weight);
                    jes_ratio->Fill(truthlead.Pt(),lead.Pt()/truthlead.Pt(), weight);
                    if (choice > 0.5) {
                        hTruthPTHalf->Fill(truthlead.Pt(), weight);
                        resp_half->Fill(lead.Pt(),truthlead.Pt(), weight);
                    } else {
                        hMeasPTHalf->Fill(lead.Pt(), weight);
                    }
                } else {
                    // FAKE AND MISS
                    hTruthPT->Fill(truthlead.Pt(), weight);
                    resp_full->Miss(truthlead.Pt(), weight);
                    if (choice > 0.5) {
                        hTruthPTHalf->Fill(truthlead.Pt(), weight);
                        resp_half->Miss(truthlead.Pt(), weight);
                    }
                    hMeasPT->Fill(lead.Pt(), weight);
                    resp_full->Fake(lead.Pt(), weight);
                    if (choice > 0.5) {
                        resp_half->Fake(lead.Pt(), weight);
                    } else {
                        hMeasPTHalf->Fill(lead.Pt(), weight);
                    }
                }
            ...
            if ((nJet >= 2 && lead.Pt() > leadptmin && sub.Pt() > subptmin && fabs(lead.DeltaPhi(sub)) > 2.75) && (nTruthJet >= 2 && truthlead.Pt() > leadptmin && truthsub.Pt() > subptmin && fabs(truthlead.DeltaPhi(truthsub)) > 2.75)) {
                if (lead.DeltaR(truthlead) < dRMax && sub.DeltaR(truthsub) < dRMax) { // should this match be both the leading and subleading? 
                    // MATCH 
                    hInverseMeasPT->Fill(truthlead.Pt(), weight);
                    hInverseTruthPT->Fill(lead.Pt(), weight);
                    inverse_resp_full->Fill(truthlead.Pt(),lead.Pt(), weight);
                    inverse_jes_ratio->Fill(lead.Pt(),truthlead.Pt()/lead.Pt(), weight);
                    if (choice > 0.5) {
                        hInverseTruthPTHalf->Fill(lead.Pt(), weight);
                        inverse_resp_half->Fill(truthlead.Pt(),lead.Pt(), weight);
                    } else {
                        hInverseMeasPTHalf->Fill(truthlead.Pt(), weight);
                    }
                } else {
                    // FAKE AND MISS
                    hInverseTruthPT->Fill(lead.Pt(), weight);
                    inverse_resp_full->Miss(lead.Pt(), weight);
                    if (choice > 0.5) {
                        hInverseTruthPTHalf->Fill(lead.Pt(), weight);
                        inverse_resp_half->Miss(lead.Pt(), weight);
                    }
                    hInverseMeasPT->Fill(truthlead.Pt(), weight);
                    inverse_resp_full->Fake(truthlead.Pt(), weight);
                    if (choice > 0.5) {
                        inverse_resp_half->Fake(truthlead.Pt(), weight);
                    } else {
                        hInverseMeasPTHalf->Fill(truthlead.Pt(), weight);
                    }
                }
