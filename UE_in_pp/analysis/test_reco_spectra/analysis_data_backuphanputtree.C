...
////////////////////////////////////////// Main Function //////////////////////////////////////////
void analysis_data_backuphanputtree(int runnumber, string trig, int nseg, int iseg, double jet_radius)  {
  ...
  TFile *f_out = new TFile(Form("output/test_hanpu_%s_output_r0%d_%d_%d_%d.root", trig.c_str(), jet_radius_index, runnumber, nseg, iseg),"RECREATE");
  //TFile *f_out = new TFile(Form("hanpu_%s_output_r0%d_%d_%d_%d.root", trig.c_str(), jet_radius_index, runnumber, nseg, iseg),"RECREATE");

  /////////////// Read Files ///////////////
  const char* baseDirJet = "/sphenix/tg/tg01/jets/hanpuj";
  TChain chain("ttree");
  //for (int i = iseg; i < iseg + nseg; ++i) {
  //  chain.Add(Form("%s/Run%d/OutDir%d/output_data.root", baseDirJet, runnumber, i));
  //}
  //chain.Add(Form("%s/CaloDataAna_ppg09/Run%d/OutDir*/output_data.root", baseDirJet, runnumber));
  for (int i = nseg; i < iseg; i++) {
    chain.Add(Form("%s/JES_MC_run%d/Jet%sGeV/OutDir%d/output_sim.root", baseDirJet, runnumber, trig.c_str(), i));
  }
  ...
  TH1D* h_leadingunsubjet_eta = new TH1D("h_leadingunsubjet_eta", ";Leading Uncalibrated Jet #eta", 100, -5.0, 5.0);
  TH1D* h_leadingtruthjet_eta = new TH1D("h_leadingtruthjet_eta", ";Leading Truth Jet #eta", 100, -5.0, 5.0);
  TH1D* h_leadingunsubjet_phi = new TH1D("h_leadingunsubjet_phi", ";Leading Uncalibrated Jet #phi", 100, -2*M_PI, 2*M_PI);
  TH1D* h_leadingtruthjet_phi = new TH1D("h_leadingtruthjet_phi", ";Leading Truth Jet #phi", 100, -2*M_PI, 2*M_PI);
  TH1D* h_zvertex = new TH1D("h_zvertex",";z_vertex [cm]", 400, -100, 100);
  TH1D* h_leadingtruthjet_zvertex = new TH1D("h_leadingtruthjet_zvertex",";z_vertex [cm]", 400, -100, 100);
  ...
  for (Long64_t ie = 0; ie < n_events; ++ie) { // event loop start
    ...
    if (runnumber > 30) {
      if (gl1_trigger_vector_scaled[22] == 0) continue;
    }
    ...
    if (leadingtruthjet_index >= 0) {
      if (truthjet_pt->at(leadingtruthjet_index) >= truthjet_pt_min && truthjet_pt->at(leadingtruthjet_index) <= truthjet_pt_max) {
        ...
        if (leadingtruthjet_index >= 0 && truthjet_pt->at(leadingtruthjet_index) > 8) { 
          ...
          h_leadingtruthjet_eta->Fill(truthjet_eta->at(leadingtruthjet_index));
          h_leadingtruthjet_phi->Fill(truthjet_phi->at(leadingtruthjet_index));
          h_leadingtruthjet_zvertex->Fill(zvertex);
    ...
    h_leadingunsubjet_eta->Fill(unsubjet_eta->at(leadingunsubjet_index));
    h_leadingunsubjet_phi->Fill(unsubjet_phi->at(leadingunsubjet_index));
  ...
  h_leadingunsubjet_eta->Write();
  h_leadingtruthjet_eta->Write();
  h_leadingunsubjet_phi->Write();
  h_leadingtruthjet_phi->Write();
  h_zvertex->Write();
  h_leadingtruthjet_zvertex->Write();
  f_out->Close();
