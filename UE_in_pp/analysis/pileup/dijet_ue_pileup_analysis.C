...
void dijet_ue_pileup_analysis(bool sim = true, bool clusters = true, bool emcal_clusters = false, bool applyCorr = false) {
    ...
    for (int pb = 1; pb < pileup_bins.size(); pb++) {
    	...
    	outfilename += "_leadjet_15_20_GeV_zvtx_lt_10cm_no_low_ET_towers_rate_range_" + to_string(pileup_bins[pb-1]) + "_" + to_string(pileup_bins[pb]) + ".root";
		...
		TH1F* h_vz = new TH1F("h_vz","",400, -100, 100);
	 	...
	 	for (int i = 0; i < run_numbers.size(); i++) {
	 		if (pileup_rates[i] < pileup_bins[pb] && pileup_rates[i] > pileup_bins[pb-1]) {
	 			std::string wildcardPath = "/sphenix/tg/tg01/jets/egm2153/UEinppOutput/output1.5mrad_ana468_" + std::to_string(run_numbers[i]) + "_*.root";
