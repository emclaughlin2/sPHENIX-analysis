...
void jet_0mrad_ue_pileup_analysis(bool sim = true, bool clusters = true, bool emcal_clusters = false, bool applyCorr = false, bool do_bkg_cut = true, bool dijet_bkg_cut = true, bool do_jet_pt_range = true) {
    ...
    for (int pb = 1; pb < pileup_bins.size(); pb++) {
    	string outfilename = "pileup_inclusive_jet_0mrad_UE_analysis";
    	...
    	outfilename += "_maxrate_" + to_string(pileup_bins[pb]) +".root";
	...
	if (!emcal_clusters) {
	    ...
	    //for (Long64_t entry = 0; entry < 1000000; ++entry) {
	    	...
	    	if (do_jet_pt_range && (lead.Pt() < 15.0 || lead.Pt() > 20.0)) { continue; }
