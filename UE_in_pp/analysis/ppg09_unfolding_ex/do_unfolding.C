...
void do_unfolding() {
  ...
  //TF1* f_mbd_efficiency = new TF1("f_mbd_efficiency", "[0]*x*x + [1]*x + [2]", 0, 100); f_mbd_efficiency->SetParameters(-7.08943e-05, -0.00270189, 0.730093);
  //TF1* f_mbd_efficiency_down = new TF1("f_mbd_efficiency_down", "[0]*x*x + [1]*x + [2]", 0, 100); f_mbd_efficiency_down->SetParameters(-7.45922e-05, -0.00242575, 0.742911);
  //TF1* f_mbd_efficiency_up = new TF1("f_mbd_efficiency_up", "[0]*x*x + [1]*x + [2]", 0, 100); f_mbd_efficiency_up->SetParameters(-6.5823e-05, -0.00304993, 0.718084);
  ...
  for (int ib = 1 + n_underflow_bin; ib <= h_unfold_calib_dijet_reweighted_1->GetNbinsX() - n_overflow_bin; ++ib) {
    double mbd_efficiency = h_mbd_efficiency->GetBinContent(ib - n_underflow_bin) / 0.57 * (25.2 / 42.);
    ...
    double mbd_efficiency_down = h_mbd_efficiency_down->GetBinContent(ib - n_underflow_bin) / 0.57 * (25.2 / 42.);
    ...
    double mbd_efficiency_up = h_mbd_efficiency_up->GetBinContent(ib - n_underflow_bin) / 0.57 * (25.2 / 42.);
