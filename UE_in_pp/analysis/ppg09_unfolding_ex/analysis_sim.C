...
void get_truthjet(std::vector<float>& goodtruthjet_pt, std::vector<float>& goodtruthjet_eta, std::vector<float>& goodtruthjet_phi, std::vector<bool>& truthjet_filter, std::vector<float>* jet_pt, std::vector<float>* jet_eta, std::vector<float>* jet_phi, int leadingtruthjet_index);
...
////////////////////////////////////////// Main Function //////////////////////////////////////////
void analysis_sim(std::string runtype, int start_seg, int end_seg)  {
  ////////// General Set up //////////
  ...
  for (int ie = 0; ie < n_events; ++ie) { // event loop start
    ...
    if (!truthjet_pt) {
	    std::cout << "truthjet_pt vector null on event " << ie << std::endl;
    	continue;
    }
    ...
    if (!unsubjet_pt) {
	    std::cout << "unsubjet_pt vector null on event " << ie << std::endl;
	    continue;
    }
    ...
    get_truthjet(goodtruthjet_pt, goodtruthjet_eta, goodtruthjet_phi, truthjet_filter, truthjet_pt, truthjet_eta, truthjet_phi, leadingtruthjet_index);
    std::cout << goodtruthjet_pt.size() << " " << goodtruthjet_eta.size() << " " << goodtruthjet_phi.size() << std::endl;
...
void get_truthjet(std::vector<float>& goodtruthjet_pt, std::vector<float>& goodtruthjet_eta, std::vector<float>& goodtruthjet_phi, std::vector<bool>& truthjet_filter, std::vector<float>* jet_pt, std::vector<float>* jet_eta, std::vector<float>* jet_phi, int leadingtruthjet_index) {
  ...
  for (int ij = 0; ij < truthjet_filter.size(); ++ij) {
    ...
    if (ij != leadingtruthjet_index) continue;
