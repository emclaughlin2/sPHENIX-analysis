...
void mbd_vertex_eff_sim(std::string runtype = "mb", int start_seg = 0, int end_seg = 200, std::string bkg_cut = "dijet", float zvertex_cut = 20.0, float wide_zvertex_cut = 100.0)  {
    ...
    ////////// Files //////////
    string outfilename = "analysis_sim_run28_output/output_sim_zvtx_lt_" + to_string(floor(zvertex_cut)) + "_" + to_string(floor(wide_zvertex_cut)) + "_" + runtype + "_" + bkg_cut + "_bkg_cut_" + to_string(start_seg) + "_" + to_string(end_seg) + ".root";
    TFile *f_out = new TFile(outfilename.c_str(), "RECREATE");
    TChain chain("T");
    for (int i = start_seg; i < end_seg; ++i) {
        chain.Add(Form("/sphenix/tg/tg01/jets/egm2153/UEinppOutput/sim_run28_%s_mbd_eff_output_%d.root", runtype.c_str(), i));
    }
    ...
    float truthvertex; chain.SetBranchStatus("ztruthvtx", 1); chain.SetBranchAddress("ztruthvtx", &truthvertex);
    ...
    double tight_truthptbins[] = {17, 22, 24, 26, 28, 30, 32, 34, 36, 38, 40, 42, 44, 46, 48, 50, 53, 56, 59, 63.5, 72, 82};
    int tight_truthnpt = sizeof(tight_truthptbins) / sizeof(tight_truthptbins[0]) - 1;
    ...
    TH2D* h_truth_mbd_corr = new TH2D("h_truth_mbd_corr",";Truth z-vertex [cm]; MBD z-vertex [cm]", 400, -200, 200, 400, -200, 200);
    TH2D* h_truth_track_corr = new TH2D("h_truth_track_corr",";Truth z-vertex [cm]; Track z-vertex [cm]", 400, -200, 200, 400, -200, 200);
    TH2D* h_track_mbd_corr = new TH2D("h_track_mbd_corr",";Track z-vertex [cm]; MBD z-vertex [cm]", 400, -200, 200, 400, -200, 200);
    TH2D* h_mbd_track_corr = new TH2D("h_mbd_track_corr",";Track z-vertex [cm]; MBD z-vertex [cm]", 400, -200, 200, 400, -200, 200);
    
    TH2D* h_total_truthvertex = new TH2D("h_total_truthvertex","", truthnpt, truthptbins, truthnet, truthetbins);
    TH2D* h_pass_cut_mbdvertex_truthvertex = new TH2D("h_pass_cut_mbdvertex_truthvertex","", truthnpt, truthptbins, truthnet, truthetbins);
    TH2D* h_pass_cut_trackvertex_truthvertex = new TH2D("h_pass_cut_trackvertex_truthvertex","", truthnpt, truthptbins, truthnet, truthetbins);
    TH2D* h_total_trackvertex = new TH2D("h_total_trackvertex","", truthnpt, truthptbins, truthnet, truthetbins);
    TH2D* h_pass_cut_mbdvertex_trackvertex = new TH2D("h_pass_cut_mbdvertex_trackvertex","", truthnpt, truthptbins, truthnet, truthetbins);
    TH2D* h_total_mbdvertex = new TH2D("h_total_mbdvertex","",truthnpt, truthptbins, truthnet, truthetbins);
    TH2D* h_pass_cut_trackvertex_mbdvertex = new TH2D("h_pass_cut_trackvertex_mbdvertex","",truthnpt, truthptbins, truthnet, truthetbins);
    
    TH1D* h_totalet_total_truthvertex = new TH1D("h_totalet_total_truthvertex", "", 440, -20, 200);
    TH1D* h_totalet_pass_cut_mbdvertex_truthvertex = new TH1D("h_totalet_pass_cut_mbdvertex_truthvertex", "", 440, -20, 200);
    TH1D* h_totalet_pass_cut_trackvertex_truthvertex = new TH1D("h_totalet_pass_cut_trackvertex_truthvertex", "", 440, -20, 200);
    TH1D* h_totalet_total_trackvertex = new TH1D("h_totalet_total_trackvertex", "", 440, -20, 200);
    TH1D* h_totalet_total_mbdvertex = new TH1D("h_totalet_total_mbdvertex", "", 440, -20, 200);
    TH1D* h_totalet_pass_cut_mbdvertex_trackvertex = new TH1D("h_totalet_pass_cut_mbdvertex_trackvertex", "", 440, -20, 200);
    TH1D* h_totalet_pass_cut_trackvertex_mbdvertex = new TH1D("h_totalet_pass_cut_trackvertex_mbdvertex", "", 440, -20, 200);
    TH1D* h_totalet_no_pass_cut_mbdvertex_truthvertex = new TH1D("h_totalet_no_pass_cut_mbdvertex_truthvertex", "", 440, -20, 200);
    TH1D* h_totalet_no_pass_cut_trackvertex_truthvertex = new TH1D("h_totalet_no_pass_cut_trackvertex_truthvertex", "", 440, -20, 200);
    TH1D* h_totalet_no_pass_cut_mbdvertex_trackvertex = new TH1D("h_totalet_no_pass_cut_mbdvertex_trackvertex", "", 440, -20, 200);
    TH1D* h_totalet_no_pass_cut_trackvertex_mbdvertex = new TH1D("h_totalet_no_pass_cut_trackvertex_mbdvertex", "", 440, -20, 200);
    ...
    //for (Long64_t entry = 0; entry < 2000; ++entry) {
        ...
        if (fabs(truthvertex) > zvertex_cut && fabs(trackvertex) > zvertex_cut && fabs(zvertex) > zvertex_cut) { continue;}
        ...
        for (size_t i = 0; i < truthjet_eta->size(); ++i) {
            if (fabs(truthjet_eta->at(i)) > 0.7) { continue; }
        ...
        *truthjet_phi = std::move(truthphi_new);
        ...
        // << "Reco lead: pt " << lead.Pt() << " e " << lead_e << " eta " << lead.Eta() << " phi " << lead.Phi()  << " bkg_cut " << reco_bkg_cut << std::endl;
        ...
        for (int i = 0; i < truthpar_n; i++) {
            ...
            truth_total_et += truthpar_e[i]; ///cosh(truthpar_eta[i]);
        ...
        //std::cout << "Vertex info: truth " << truthvertex << " track " << trackvertex << " mbd " << zvertex << std::endl;
        //std::cout << std::endl;
        // Create efficiency plots here
        if (goodtruthjet_pt >= truthptbins[0] && goodtruthjet_pt < truthptbins[truthnpt]) {
            if (fabs(truthvertex) < zvertex_cut) {
                h_total_truthvertex->Fill(goodtruthjet_pt, truth_et_transverse, weight_scale);
                h_totalet_total_truthvertex->Fill(truth_total_et, weight_scale);
                if (fabs(zvertex) < wide_zvertex_cut) {
                    h_truth_mbd_corr->Fill(truthvertex, zvertex, weight_scale);
                    h_pass_cut_mbdvertex_truthvertex->Fill(goodtruthjet_pt, truth_et_transverse, weight_scale);
                    h_totalet_pass_cut_mbdvertex_truthvertex->Fill(truth_total_et, weight_scale);
                } else {
                    h_totalet_no_pass_cut_mbdvertex_truthvertex->Fill(truth_total_et, weight_scale);
                }
                if (fabs(trackvertex) < wide_zvertex_cut) {
                    h_truth_track_corr->Fill(truthvertex, trackvertex, weight_scale);
                    h_pass_cut_trackvertex_truthvertex->Fill(goodtruthjet_pt, truth_et_transverse, weight_scale);
                    h_totalet_pass_cut_trackvertex_truthvertex->Fill(truth_total_et, weight_scale);
                } else {
                    h_totalet_no_pass_cut_trackvertex_truthvertex->Fill(truth_total_et, weight_scale);
                }
            }
            if (fabs(trackvertex) < zvertex_cut) {
                h_total_trackvertex->Fill(goodtruthjet_pt, truth_et_transverse, weight_scale);
                h_totalet_total_trackvertex->Fill(truth_total_et, weight_scale);
                if (fabs(zvertex) < wide_zvertex_cut) {
                    h_track_mbd_corr->Fill(trackvertex, zvertex, weight_scale);
                    h_pass_cut_mbdvertex_trackvertex->Fill(goodtruthjet_pt, truth_et_transverse, weight_scale);
                    h_totalet_pass_cut_mbdvertex_trackvertex->Fill(truth_total_et, weight_scale);
                } else {
                    h_totalet_no_pass_cut_mbdvertex_trackvertex->Fill(truth_total_et, weight_scale);
                }
            }
            if (fabs(zvertex) < zvertex_cut) {
                h_total_mbdvertex->Fill(goodtruthjet_pt, truth_et_transverse, weight_scale);
                h_totalet_total_mbdvertex->Fill(truth_total_et, weight_scale);
                if (fabs(trackvertex) < wide_zvertex_cut) {
                    h_mbd_track_corr->Fill(trackvertex, zvertex, weight_scale);
                    h_pass_cut_trackvertex_mbdvertex->Fill(goodtruthjet_pt, truth_et_transverse, weight_scale);
                    h_totalet_pass_cut_trackvertex_mbdvertex->Fill(truth_total_et, weight_scale);
                } else {
                    h_totalet_no_pass_cut_trackvertex_mbdvertex->Fill(truth_total_et, weight_scale);
                }
            }
        }
        ...
        if (calibjet_pt_dijet >= calibptbins[0] && calibjet_pt_dijet < calibptbins[calibnpt] && reco_bkg_cut) {
            if (fabs(trackvertex) < zvertex_cut) {
                h_total_measure_trackvertex->Fill(calibjet_pt_dijet, et_transverse, weight_scale);
                h_totalet_total_measure_trackvertex->Fill(total_et, weight_scale);
                h_totalet_calibjet_total_measure_trackvertex->Fill(calibjet_pt_dijet, total_et, weight_scale);
                if (fabs(zvertex) < wide_zvertex_cut) {
                    h_measure_track_mbd_corr->Fill(trackvertex, zvertex, weight_scale);
                    h_measure_track_mbd_diff->Fill(zvertex - trackvertex, weight_scale);
                    h_pass_cut_measure_mbdvertex_trackvertex->Fill(calibjet_pt_dijet, et_transverse, weight_scale);
                    h_totalet_pass_cut_measure_mbdvertex_trackvertex->Fill(total_et, weight_scale);
                    h_totalet_calibjet_pass_cut_measure_mbdvertex_trackvertex->Fill(calibjet_pt_dijet, total_et, weight_scale);
                } else {
                    ...
                    //}
                    h_totalet_no_pass_cut_measure_mbdvertex_trackvertex->Fill(total_et, weight_scale);
                    h_totalet_calibjet_no_pass_cut_measure_mbdvertex_trackvertex->Fill(calibjet_pt_dijet, total_et, weight_scale);
                }
                if (fabs(siliconvertex) < wide_zvertex_cut) {
                    h_measure_track_silicon_corr->Fill(siliconvertex, trackvertex, weight_scale);
                    h_measure_track_silicon_diff->Fill(trackvertex - siliconvertex, weight_scale);
                    h_pass_cut_measure_siliconvertex_trackvertex->Fill(calibjet_pt_dijet, et_transverse, weight_scale);
                    h_totalet_pass_cut_measure_siliconvertex_trackvertex->Fill(total_et, weight_scale);
                    h_totalet_calibjet_pass_cut_measure_siliconvertex_trackvertex->Fill(calibjet_pt_dijet, total_et, weight_scale);
                } else {
                    //std::cout << "reco jet pt " << calibjet_pt_dijet << " eta " << calibjet_eta_dijet << " phi " << calibjet_phi_dijet << " vz_silicon " << zvertex << std::endl;
                    //for (int i = 0; i < clsmult; i++) {
                    //    std::cout << "\t cluster e " << cluster_e[i] << " eta " << cluster_eta[i] << " phi " << cluster_phi[i] << std::endl;
                    //}
                    h_totalet_no_pass_cut_measure_siliconvertex_trackvertex->Fill(total_et, weight_scale);
                    h_totalet_calibjet_no_pass_cut_measure_siliconvertex_trackvertex->Fill(calibjet_pt_dijet, total_et, weight_scale);
                }   
            }
            if (fabs(siliconvertex) < zvertex_cut) {
                h_total_measure_siliconvertex->Fill(calibjet_pt_dijet, et_transverse, weight_scale);
                h_totalet_total_measure_siliconvertex->Fill(total_et, weight_scale);
                h_totalet_calibjet_total_measure_siliconvertex->Fill(calibjet_pt_dijet, total_et, weight_scale);
                if (fabs(zvertex) < wide_zvertex_cut) {
                    h_measure_silicon_mbd_corr->Fill(siliconvertex, zvertex, weight_scale);
                    h_measure_silicon_mbd_diff->Fill(zvertex - siliconvertex, weight_scale);
                    h_pass_cut_measure_mbdvertex_siliconvertex->Fill(calibjet_pt_dijet, et_transverse, weight_scale);
                    h_totalet_pass_cut_measure_mbdvertex_siliconvertex->Fill(total_et, weight_scale);
                    h_totalet_calibjet_pass_cut_measure_mbdvertex_siliconvertex->Fill(calibjet_pt_dijet, total_et, weight_scale);
                } else {
                    //std::cout << "reco jet pt " << calibjet_pt_dijet << " eta " << calibjet_eta_dijet << " phi " << calibjet_phi_dijet << " vz_mbd " << zvertex << std::endl;
                    //for (int i = 0; i < clsmult; i++) {
                    //    std::cout << "\t cluster e " << cluster_e[i] << " eta " << cluster_eta[i] << " phi " << cluster_phi[i] << std::endl;
                    //}
                    h_totalet_no_pass_cut_measure_mbdvertex_siliconvertex->Fill(total_et, weight_scale);
                    h_totalet_calibjet_no_pass_cut_measure_mbdvertex_siliconvertex->Fill(calibjet_pt_dijet, total_et, weight_scale);
                }
                if (fabs(trackvertex) < wide_zvertex_cut) {
                    h_measure_silicon_track_corr->Fill(siliconvertex, trackvertex, weight_scale);
                    h_measure_silicon_track_diff->Fill(trackvertex - siliconvertex, weight_scale);
                    h_pass_cut_measure_trackvertex_siliconvertex->Fill(calibjet_pt_dijet, et_transverse, weight_scale);
                    h_totalet_pass_cut_measure_trackvertex_siliconvertex->Fill(total_et, weight_scale);
                    h_totalet_calibjet_pass_cut_measure_trackvertex_siliconvertex->Fill(calibjet_pt_dijet, total_et, weight_scale);
                    int ntracks = 0;
                    for (int i = 0; i < trkmult; i++) {
                        if (tr_nintt[i] > 1 && tr_ntpc[i] > 25 && tr_crossing[i] == 0) {
                            h_track_pt_spectra_pass_cut_measure_trackvertex_siliconvertex->Fill(tr_pt[i], weight_scale);
                            ntracks++;
                        }
                    }
                    h_track_mult_pass_cut_measure_trackvertex_siliconvertex->Fill(ntracks, weight_scale);
                } else {
                    int ntracks = 0;
                    for (int i = 0; i < trkmult; i++) {
                        if (tr_nintt[i] > 1 && tr_ntpc[i] > 25 && tr_crossing[i] == 0) {
                            h_track_pt_spectra_no_pass_cut_measure_trackvertex_siliconvertex->Fill(tr_pt[i], weight_scale);
                            ntracks++;
                        }
                    }
                    h_track_mult_no_pass_cut_measure_trackvertex_siliconvertex->Fill(ntracks, weight_scale);
                    h_totalet_no_pass_cut_measure_trackvertex_siliconvertex->Fill(total_et, weight_scale);
                    h_totalet_calibjet_no_pass_cut_measure_trackvertex_siliconvertex->Fill(calibjet_pt_dijet, total_et, weight_scale);
                }
            }
            if (fabs(zvertex) < zvertex_cut) {
                h_total_measure_mbdvertex->Fill(calibjet_pt_dijet, et_transverse, weight_scale);
                h_totalet_total_measure_mbdvertex->Fill(total_et, weight_scale);
                h_totalet_calibjet_total_measure_mbdvertex->Fill(calibjet_pt_dijet, total_et, weight_scale);
                if (fabs(trackvertex) < wide_zvertex_cut) {
                    h_measure_mbd_track_corr->Fill(trackvertex, zvertex, weight_scale);
                    h_measure_mbd_track_diff->Fill(zvertex - trackvertex, weight_scale);
                    h_pass_cut_measure_trackvertex_mbdvertex->Fill(calibjet_pt_dijet, et_transverse, weight_scale);
                    h_totalet_pass_cut_measure_trackvertex_mbdvertex->Fill(total_et, weight_scale);
                    h_totalet_calibjet_pass_cut_measure_trackvertex_mbdvertex->Fill(calibjet_pt_dijet, total_et, weight_scale);
                    int ntracks = 0;
                    for (int i = 0; i < trkmult; i++) {
                        if (tr_nintt[i] > 1 && tr_ntpc[i] > 25 && tr_crossing[i] == 0) {
                            h_track_pt_spectra_pass_cut_measure_trackvertex_mbdvertex->Fill(tr_pt[i], weight_scale);
                            ntracks++;
                        }
                    }
                    h_track_mult_pass_cut_measure_trackvertex_mbdvertex->Fill(ntracks, weight_scale);
                } else {
                    int ntracks = 0;
                    for (int i = 0; i < trkmult; i++) {
                        if (tr_nintt[i] > 1 && tr_ntpc[i] > 25 && tr_crossing[i] == 0) {
                            h_track_pt_spectra_no_pass_cut_measure_trackvertex_mbdvertex->Fill(tr_pt[i], weight_scale);
                            ntracks++;
                        }
                    }
                    h_track_mult_no_pass_cut_measure_trackvertex_mbdvertex->Fill(ntracks, weight_scale);
                    h_totalet_no_pass_cut_measure_trackvertex_mbdvertex->Fill(total_et, weight_scale);
                    h_totalet_calibjet_no_pass_cut_measure_trackvertex_mbdvertex->Fill(calibjet_pt_dijet, total_et, weight_scale);
                }
                if (fabs(siliconvertex) < wide_zvertex_cut) {
                    h_measure_mbd_silicon_corr->Fill(siliconvertex, zvertex, weight_scale);
                    h_measure_mbd_silicon_diff->Fill(zvertex - siliconvertex, weight_scale);
                    h_pass_cut_measure_siliconvertex_mbdvertex->Fill(calibjet_pt_dijet, et_transverse, weight_scale);
                    h_totalet_pass_cut_measure_siliconvertex_mbdvertex->Fill(total_et, weight_scale);
                    h_totalet_calibjet_pass_cut_measure_siliconvertex_mbdvertex->Fill(calibjet_pt_dijet, total_et, weight_scale);
                    int ntracks = 0;
                    for (int i = 0; i < trkmult; i++) {
                        if (tr_nintt[i] > 1 && tr_ntpc[i] > 25 && tr_crossing[i] == 0) {
                            h_track_pt_spectra_pass_cut_measure_siliconvertex_mbdvertex->Fill(tr_pt[i], weight_scale);
                            ntracks++;
                        }
                    }
                    h_track_mult_pass_cut_measure_siliconvertex_mbdvertex->Fill(ntracks, weight_scale);
                } else {
                    int ntracks = 0;
                    for (int i = 0; i < trkmult; i++) {
                        if (tr_nintt[i] > 1 && tr_ntpc[i] > 25 && tr_crossing[i] == 0) {
                            h_track_pt_spectra_no_pass_cut_measure_siliconvertex_mbdvertex->Fill(tr_pt[i], weight_scale);
                            ntracks++;
                        }
                    }
                    h_track_mult_no_pass_cut_measure_siliconvertex_mbdvertex->Fill(ntracks, weight_scale);
                    h_totalet_no_pass_cut_measure_siliconvertex_mbdvertex->Fill(total_et, weight_scale);
                    h_totalet_calibjet_no_pass_cut_measure_siliconvertex_mbdvertex->Fill(calibjet_pt_dijet, total_et, weight_scale);
                }
            }
        }
    }
    ...
    h_jes_qa->Write(); h_truth_qa->Write(); h_measure_qa->Write(); efficiency_truth_match_test->Write();
    ...
    h_total_truthvertex->Write(); h_total_mbdvertex->Write(); h_total_trackvertex->Write(); 
    h_pass_cut_mbdvertex_truthvertex->Write(); h_pass_cut_trackvertex_truthvertex->Write(); h_pass_cut_mbdvertex_trackvertex->Write(); h_pass_cut_trackvertex_mbdvertex->Write();
    h_truth_mbd_corr->Write(); h_truth_track_corr->Write(); h_track_mbd_corr->Write(); h_mbd_track_corr->Write(); 
    h_totalet_total_truthvertex->Write(); h_totalet_pass_cut_mbdvertex_truthvertex->Write(); h_totalet_pass_cut_trackvertex_truthvertex->Write(); 
    h_totalet_total_trackvertex->Write(); h_totalet_total_mbdvertex->Write(); h_totalet_pass_cut_mbdvertex_trackvertex->Write(); h_totalet_pass_cut_trackvertex_mbdvertex->Write(); 
    h_totalet_no_pass_cut_mbdvertex_truthvertex->Write(); h_totalet_no_pass_cut_trackvertex_truthvertex->Write(); h_totalet_no_pass_cut_mbdvertex_trackvertex->Write(); h_totalet_no_pass_cut_trackvertex_mbdvertex->Write();
