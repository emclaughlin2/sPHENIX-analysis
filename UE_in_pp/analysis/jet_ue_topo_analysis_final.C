...
void jet_ue_topo_analysis_final(string infilename = "sim_run21_jet10_output_*.root", string outfilename = "dijet_calo_analysis_run21_jet10_topocluster.root", bool sim = false, bool clusters = true, bool emcal_clusters = false, bool applyCorr = false, bool do_bkg_cut = true, bool dijet_bkg_cut = true, bool do_truth_match = true) {
  ...
  TF1 *correction;
  if(applyCorr) {
      	corrFile = new TFile("unfolding/JES_Calib_Default.root","READ");
      	corrFile -> cd();
      	correction = (TF1*)corrFile -> Get("JES_Calib_Default_Func");
  }
    ...
    //for (Long64_t entry = 0; entry < 20000; ++entry) {
      ...
      //if (check_bad_jet_eta((*eta)[ind_lead], zvertex, jet_radius) || (*pt)[ind_lead] < lead_ptmin) { continue; }
  		...
  		if (do_bkg_cut) {
	  		if (dijet_bkg_cut) {
	  			...
	  			if ((*pt)[ind_sub] < 0.3*(*pt)[ind_lead]) { continue; }
	  			TVector3 ilead, isub;
	  			ilead.SetPtEtaPhi((*pt)[ind_lead], (*eta)[ind_lead], (*phi)[ind_lead]);
    			isub.SetPtEtaPhi((*pt)[ind_sub], (*eta)[ind_sub], (*phi)[ind_sub]);
  				if (fabs(ilead.DeltaPhi(isub)) < deltaphimin) { continue; }
	  		} else {
		  		...
		  		temp_sub = 0;
	  			for (int i = 0; i < nJet; i++) {
		  			if ((*pt)[i] > temp_lead && !check_bad_jet_eta((*eta)[ind_lead], zvtx, jet_radius)) {
		  				if (temp_lead != 0) {
		  					temp_sub = temp_lead;
		  					ind_sub = ind_lead;
		  				}
		  				temp_lead = (*pt)[i];
		  				ind_lead = i;
		  			} else if ((*pt)[i] > temp_sub && !check_bad_jet_eta((*eta)[ind_lead], zvtx, jet_radius)) {
		  				temp_sub = (*pt)[i];
		  				ind_sub = i;
		  			}
	  			}
  		...
  		temp_sub = 0;
			...
			if ((*pt)[ind_lead] < lead_ptmin) { continue; }

  		// create leading and subleading jet vectors and apply JES calibration correction if applicable 
  		TVector3 lead, sub;
  		if (applyCorr) {
  			double lead_calib_pt = correction->Eval((*pt)[ind_lead]) * (1 + randGen.Gaus(0.0, 0.1)) * 1.0;
  			lead.SetPtEtaPhi(lead_calib_pt, (*eta)[ind_lead], (*phi)[ind_lead]);
  			if (dijet_bkg_cut && ind_sub != 0) {
  				double sub_calib_pt = correction->Eval((*pt)[ind_sub]) * (1 + randGen.Gaus(0.0, 0.1)) * 1.0;
  				sub.SetPtEtaPhi(sub_calib_pt, (*eta)[ind_sub], (*phi)[ind_sub]);
  			}
    	} else {
    		lead.SetPtEtaPhi((*pt)[ind_lead], (*eta)[ind_lead], (*phi)[ind_lead]);
    		if (dijet_bkg_cut && ind_sub != 0) sub.SetPtEtaPhi((*pt)[ind_sub], (*eta)[ind_sub], (*phi)[ind_sub]);
    	}

    	//std::cout << "lead pt " << lead.Pt() << " sub pt " << sub.Pt() << std::endl;
    	...
    	if (sim && do_truth_match) {
    		...
    		if (dijet_bkg_cut && ind_sub != 0 && reco_matched[ind_sub] == -1) { continue; }
    	}
