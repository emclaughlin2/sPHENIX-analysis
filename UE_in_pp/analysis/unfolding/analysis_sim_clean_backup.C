...
void analysis_sim_clean(std::string runtype = "mb", int start_seg = 0, int end_seg = 200, int iter = 1, std::string bkg_cut = "dijet", bool clusters = true, bool emcal_clusters = false)  {
    ...
    if (runtype == "mb") {
        ...
        truthjet_pt_min = 14;
        //truthjet_pt_max = 17;  //edited for run21 testing
        truthjet_pt_max = 22;
        if (iter > 1) count_filename = "analysis_sim_run28_output/test_run21_output_analysis_clean_" + bkg_cut + "_bkg_cut_sim_iter_1_jet10.root";
        ...
        truthjet_pt_min = 22;
        truthjet_pt_max = 35;
        if (iter > 1) count_filename = "analysis_sim_run28_output/test_run21_output_analysis_clean_" + bkg_cut + "_bkg_cut_sim_iter_1_jet20.root";
    } else if (runtype == "jet30") {
        weight_scale = Jet30GeV_scale;
        truthjet_pt_min = 35;
        truthjet_pt_max = 52;
        if (iter > 1) count_filename = "analysis_sim_run28_output/test_run21_output_analysis_clean_" + bkg_cut + "_bkg_cut_sim_iter_1_jet30.root";
    } else if (runtype == "jet50") {
        weight_scale = Jet50GeV_scale;
        truthjet_pt_min = 52;
        //truthjet_pt_max = 71; //edited for run21 testing
        truthjet_pt_max = 1000; 
        if (iter > 1) count_filename = "analysis_sim_run28_output/test_run21_output_analysis_clean_" + bkg_cut + "_bkg_cut_sim_iter_1_jet50.root";
    ...
    if (iter > 2) {
        if (runtype == "herwig_jet10" || runtype == "herwig_jet30") {
            ...
            std::string rwfilename = "run28_output_files/test_run21_output_analysis_clean_reweighted_respmatrix_iter_2_"+bkg_cut+"_bkg_cut.root";
    ...
    string outfilename = "analysis_sim_run28_output/test_run21_output_analysis_clean_" + bkg_cut + "_bkg_cut_sim_iter_" + to_string(iter) + "_" + runtype + "_" + to_string(start_seg) + "_" + to_string(end_seg) + ".root"; // edited for run21 testing
    TFile *f_out = new TFile(outfilename.c_str(), "RECREATE");
    TChain chain("T");
    for (int i = start_seg; i < end_seg; ++i) {
        chain.Add(Form("/sphenix/tg/tg01/jets/egm2153/UEinppOutput/sim_run21_%s_3sigma_output_%d.root", runtype.c_str(), i)); // edited for run21 testing 
        //chain.Add(Form("run21_test_files/sim_run21_%s_3sigma_output_%d.root", runtype.c_str(), i)); // edited for run21 testing 
        //chain.Add(Form("/sphenix/tg/tg01/jets/egm2153/UEinppOutput/sim_run28_%s_3sigma_output_%d.root", runtype.c_str(), i)); // edited for run21 testing
    }
    ...
    //for (Long64_t entry = 0; entry < 2000; ++entry) {
        ...
        if (fabs(zvertex) > 30) { continue; } // edited for unfolding procedure testing
        ...
        *truthjet_phi = std::move(truthphi_new);
        ...
        for (size_t i = 0; i < unsubjet_eta->size(); ++i) {
            //if (!check_bad_jet_eta(unsubjet_eta->at(i), zvertex, jet_radius)) { // edited for testing
        ...
        */
        ...
        if (truthjet_pt->size() >= nJetReq) {
            ...
            /*
            std::cout << " Truth pt " << truthlead.Pt() << std::endl;
            if (truthlead.Pt() < 5.0) {
                for (int i = 0; i < truthjet_pt->size(); i++) {
                    std::cout << "\t  check container for lead truth jet < 5 GeV: truth jet pt: " << truthjet_pt->at(i) << std::endl;
                }
                for (int i = 0; i < unsubjet_pt->size(); i++) {
                    std::cout << "\t  check container for lead truth jet < 5 GeV: unsub jet pt: " << unsubjet_pt->at(i) << std::endl;
                }
            }
            */
...
void get_calibjet(float& calibjet_pt, float& calibjet_eta, float& calibjet_phi, float jet_pt, float jet_eta, float jet_phi, bool reco_cut, TF1* f_corr, float jes_para, float jer_para) {
    ...
    //std::cout << " calib jet pt in get_calibjet " << calib_pt << std::endl;
...
void get_truthjet(float& goodtruthjet_pt, float& goodtruthjet_eta, float& goodtruthjet_phi, float jet_pt, float jet_eta, float jet_phi, bool truth_cut) {
    ...
    if (!truth_cut) return;
    //std::cout << " truth jet pt in get_truthjet " << jet_pt << std::endl;
