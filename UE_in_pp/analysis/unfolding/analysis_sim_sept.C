...
void analysis_sim_sept(std::string runtype = "mb", int start_seg = 0, int end_seg = 200, int iter = 1, std::string bkg_cut = "dijet", bool clusters = true, bool emcal_clusters = false)  {
    ...
    if (runtype == "mb") {
        ...
        truthjet_pt_max = 22;
        if (iter > 1) count_filename = "analysis_sim_run28_output/output_sim_iter_1_jet10.root";
    } else if (runtype == "jet15") {
        weight_scale = Jet15GeV_scale;
        truthjet_pt_min = 17;
        truthjet_pt_max = 22;
        if (iter > 1) count_filename = "analysis_sim_run28_output/output_sim_iter_1_jet15.root";
    } else if (runtype == "jet20") {
        weight_scale = Jet20GeV_scale;
        truthjet_pt_min = 22;
        truthjet_pt_max = 35;
        if (iter > 1) count_filename = "analysis_sim_run28_output/output_sim_iter_1_jet20.root";
    } else if (runtype == "jet30") {
        weight_scale = Jet30GeV_scale;
        truthjet_pt_min = 35;
        truthjet_pt_max = 52;
        if (iter > 1) count_filename = "analysis_sim_run28_output/output_sim_iter_1_jet30.root";
    } else if (runtype == "jet50") {
        weight_scale = Jet50GeV_scale;
        truthjet_pt_min = 52;
        //truthjet_pt_max = 71; //edited for run21 testing
        truthjet_pt_max = 1000; 
        if (iter > 1) count_filename = "analysis_sim_run28_output/output_sim_iter_1_jet50.root";
    } else if (runtype == "jet70") {
        weight_scale = Jet70GeV_scale;
        truthjet_pt_min = 71;
        truthjet_pt_max = 1000;
        if (iter > 1) count_filename = "analysis_sim_run28_output/output_sim_iter_1_jet70.root";
    ...
    if (iter > 2) {
        if (runtype == "herwig_jet10" || runtype == "herwig_jet30") {
            ...
            std::string rwfilename = "output_reweighted_respmatrix_run21_iter_2_"+bkg_cut+"_bkg_cut.root";
    ...
    string outfilename = "analysis_sim_run28_output/output_analysis_sim_sept_" + bkg_cut + "bkg_cut_sim_iter_" + to_string(iter) + "_" + runtype + "_" + to_string(start_seg) + "_" + to_string(end_seg) + ".root";
    ...
    */
    ...
    //for (Long64_t entry = 0; entry < 2000; ++entry) {
        ...
        */
        ...
        if (unsubjet_pt->size() < nJetReq && truthjet_pt->size() < 1) {
            continue;
        }
        ...
        get_calibjet(calibjet_pt_dijet_jesdown, calibjet_eta_dijet_jesdown, calibjet_phi_dijet_jesdown, lead.Pt(), lead.Eta(), lead.Phi(), reco_bkg_cut, f_corr, 0.94, 0.1);
        get_calibjet(calibjet_pt_dijet_jesup, calibjet_eta_dijet_jesup, calibjet_phi_dijet_jesup, lead.Pt(), lead.Eta(), lead.Phi(), reco_bkg_cut, f_corr, 1.06, 0.1);
        get_calibjet(calibjet_pt_dijet_jerdown, calibjet_eta_dijet_jerdown, calibjet_phi_dijet_jerdown, lead.Pt(), lead.Eta(), lead.Phi(), reco_bkg_cut, f_corr, 1, 0.05);
        get_calibjet(calibjet_pt_dijet_jerup, calibjet_eta_dijet_jerup, calibjet_phi_dijet_jerup, lead.Pt(), lead.Eta(), lead.Phi(), reco_bkg_cut, f_corr, 1, 0.15);
        ...
        */
        ...
        /*
        if (entry == 29 && reco_bkg_cut && calibjet_matched_dijet) {
    ...
    h_zvertex->Write();  h_jes_qa->Write(); h_truth_qa->Write(); h_measure_qa->Write();
...
void get_leading_jet(int& leadingjet_index, std::vector<float>* jet_et, std::vector<float>* jet_eta) {
  ...
  float leadingjet_et = -9999;
  for (int ij = 0; ij < jet_et->size(); ++ij) {
    if (fabs(jet_eta->at(ij)) > 0.7) { continue; }
    float jetet = jet_et->at(ij);
    if (jetet > leadingjet_et) {
      leadingjet_et = jetet;
      leadingjet_index = ij;
