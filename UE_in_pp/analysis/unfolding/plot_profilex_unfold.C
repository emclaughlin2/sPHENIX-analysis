...
void plot_profilex_unfold(const char* unfoldfile = "run28_output_files/output_unfolded_data_pu_correct_calib_dijet_run28_iter_3_1000toys.root") {
    ...
    TH1D* hc_measure = (TH1D*)h_measure_2D->ProfileX("hc_measure");
    for (int i = 0; i < 1; i++) {
        for (int j = 0; j < trim.size(); j++) {
            ...
            hc_truth[i][j] = h_truth_2D[i][j]->ProfileX(("hc_truth_"+syst[i]+trim[j]).c_str());
            for (int n = 0; n < 20; n++) {
                ...
                hc_unfold[i][j][n] = h_unfold_2D[i][j][n]->ProfileX(("hc_unfold_"+syst[i]+trim[j]+"_"+to_string(n+1)).c_str());
            }
        }
    }
    /*
    ...
    */
    ...
    for (int it = 1; it < 20; it++) {
        ...
        for (int i = 0; i < trim.size(); i++) {
            for (int x = 2; x < hc_unfold[0][i][it]->GetNbinsX(); x++) {
                sigma_error[i][it-1] += pow(hc_unfold[0][i][it]->GetBinError(x),2);
                sigma_val_diff[i][it-1] += pow(hc_unfold[0][i][it]->GetBinContent(x) - hc_unfold[0][i][it-1]->GetBinContent(x),2);
            }
        }
    }
