...
void plot_unfoldet(bool dijet = 1) 
{
    string infile;
    if (dijet) {
        infile = "sphenix_primary_run28_output_files/output_mbd_correct_unfolded_data_8calibetbin_dijet_bkg_cut_run28_iter_3_1000toys.root";
    } else {
        infile = "sphenix_primary_run28_output_files/output_mbd_correct_unfolded_data_8calibetbin_efrac_bkg_cut_run28_iter_3_1000toys.root";
    }
    ...
    std::vector<std::string> names = {
        "h_unfold_calib_dijet_reweight_trim_10_3_etEffCorrected",
        "h_unfold_h_calibjet_pt_dijet_pu_correct_et_calib_dijet_reweight_trim_10_3_etEffCorrected",
    ...
    for (size_t i = 0; i < names.size(); i++) {
        ...
        if (!h_uni) {
            Warning("plot_unfoldjets_manual","Histogram %s not found!",names[i].c_str());
            continue;
        }
        ...
        TH2D* h_var = new TH2D((names[i] + "_var").c_str(), "",
                               nx, xmin, xmax, truthnet, truthetbins);
        ...
        TH1D* py = h_var->ProjectionY((names[i] + "_py").c_str(),2,5);
    ...
    if (dijet) {
        c->SaveAs("sphenix_primary_run28_output_files/unfoldet_neg_1GeV_et_cut_dijet_et_binning_comparison.png");
    } else {
        c->SaveAs("sphenix_primary_run28_output_files/unfoldet_neg_1GeV_et_cut_efrac_et_binning_comparison.png");
    }
}
