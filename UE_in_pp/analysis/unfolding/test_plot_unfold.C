...
void draw_iteration_graph(std::vector<TGraph*> graphs, std::vector<std::string> leg_tags, std::string syst_tag, std::string syst) {
    ...
    string outfile = "plots_run28/h_unfolding_iterations_all_respmatrices_pu_correct.png";
    if (syst != "") outfile = "plots_run28/h_unfolding_iterations_" + syst + "_pu_correct.png";

    c1->SaveAs(outfile.c_str());
}

void test_plot_unfold(const char* unfoldfile = "run28_output_files/output_unfolded_data_pu_correct_calib_dijet_run28_iter_3_1000toys.root") {
    ...
    TH2D* h_uni_measure1_2D = (TH2D*)f->Get("h_calibjet_pt_dijet_eff");
    TH2D* h_measure1_2D = new TH2D("h_var_measure1","",calibnpt, calibptbins, calibnet, calibetbins);   
    for (int ix = 1; ix < h_uni_measure1_2D->GetNbinsX() + 1; ix++) {
        for (int iy = 1; iy < h_uni_measure1_2D->GetNbinsY() + 1; iy++) {
            h_measure1_2D->SetBinContent(ix,iy,h_uni_measure1_2D->GetBinContent(ix,iy));
            h_measure1_2D->SetBinError(ix,iy,h_uni_measure1_2D->GetBinError(ix,iy));
        }
    }
    TH1D* hj_measure1 = (TH1D*)h_measure1_2D->ProjectionX("hj_measure1");
    TH1D* hc_measure1 = (TH1D*)h_measure1_2D->ProjectionY("hc_measure1");
    ...
    for (int i = 0; i < 1; i++) {
        for (int j = 0; j < 6; j++) {
            ...
            for (int n = 0; n < 20; n++) {
                h_uni_unfold_2D[i][j][n] = (TH2D*)f->Get(("h_unfold_"+syst[i]+trim[j]+"_"+to_string(n+1)).c_str());
