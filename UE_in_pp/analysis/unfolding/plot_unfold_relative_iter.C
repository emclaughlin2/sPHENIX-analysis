...
void plot_unfold_relative_iter(const char* unfoldfile = "run28_output_files/output_unfolded_data_pu_correct_calib_dijet_run28_iter_3_1000toys.root") {
        ...
        for (int i = 0; i < trim.size(); i++) {
            for (int x = 2; x < h_unfold_2D[0][i][it]->GetNbinsX(); x++) {
                for (int y = 1; y < h_unfold_2D[0][i][it]->GetNbinsY(); y++) {
                    sigma_error[i][it-1] += pow(h_unfold_2D[0][i][it]->GetBinError(x,y)/h_unfold_2D[0][i][it]->GetBinContent(x,y),2);
                    sigma_val_diff[i][it-1] += pow((h_unfold_2D[0][i][it]->GetBinContent(x,y) - h_unfold_2D[0][i][it-1]->GetBinContent(x,y))/h_unfold_2D[0][i][it]->GetBinContent(x,y),2);
                }
            }
        }
    }
