...
void analysis_sim_primary_particle_effects(std::string runtype = "mb", int start_seg = 0, int end_seg = 200, int iter = 1, std::string bkg_cut = "dijet", bool clusters = true, bool emcal_clusters = false)  {
    ...
    "h_respmatrix_calib_dijet_clus_smear_counts","h_respmatrix_calib_dijet_ohcal_mc_data_var_counts","h_respmatrix_calib_dijet_2sigma_noise_counts",
    ...
    TH2D* counts_measured[10]; 
    TH2D* counts_truth[10]; 
    ...
    if (iter > 1) {
        ...
        for (int i = 0; i < count_matrix_names.size(); i++) {
            ...
            if (i < 10) {
                ...
                counts1D_measured[i-7] = dynamic_cast<TH1D*>(h_meas->Clone());
                counts1D_truth[i-7]    = dynamic_cast<TH1D*>(h_true->Clone());
                counts1D_measured[i-7]->SetDirectory(0);
                counts1D_truth[i-7]->SetDirectory(0);
            }
    ...
    */
    ...
    "weights_calib_dijet_clus_smear","weights_calib_dijet_ohcal_mc_data_var","weights_calib_dijet_2sigma_noise","weights_calib_dijet_half1","weights_calib_dijet_half2",
    ...
    if (iter > 2) {
        ...
        for (int i = 0; i < pw_syst.size(); i++) {
            for (int j = 0; j < pw_trim.size(); j++) {
                if (i < 10) { weights[i][j] = dynamic_cast<TH2D*>(f_reweight->Get((pw_syst[i]+pw_trim[j]).c_str())); }
                else { weights1D[i-10][j] = dynamic_cast<TH1D*>(f_reweight->Get((pw_syst[i]+pw_trim[j]).c_str())); }
            }
        }
    }
    ...
    string outfilename = "analysis_sim_run28_output/primary_particle_effects_output_" + bkg_cut + "_bkg_cut_sim_iter_" + to_string(iter) + "_" + runtype + "_" + to_string(start_seg) + "_" + to_string(end_seg) + ".root";
    ...
    int old_truthpar_n = 0; chain.SetBranchStatus("old_truthpar_n", 1); chain.SetBranchAddress("old_truthpar_n",&old_truthpar_n);
    float old_truthpar_e[10000] = {0.0}; chain.SetBranchStatus("old_truthpar_e", 1); chain.SetBranchAddress("old_truthpar_e",old_truthpar_e);
    float old_truthpar_eta[10000] = {0.0}; chain.SetBranchStatus("old_truthpar_eta", 1); chain.SetBranchAddress("old_truthpar_eta",old_truthpar_eta);
    float old_truthpar_phi[10000] = {0.0}; chain.SetBranchStatus("old_truthpar_phi", 1); chain.SetBranchAddress("old_truthpar_phi",old_truthpar_phi);
    int old_truthpar_pid[10000] = {0}; chain.SetBranchStatus("old_truthpar_pid", 1); chain.SetBranchAddress("old_truthpar_pid",old_truthpar_pid);
    ...
    TH1D* h_old_truth_missing_pid = new TH1D("h_old_truth_missing_pid","",10000,-5000,5000);
    TH1D* h_old_truth_missing_eta = new TH1D("h_old_truth_missing_eta","",250,-1.25,1.25);
    TH1D* h_old_truth_missing_dphi = new TH1D("h_old_truth_missing_dphi","",640,-2*M_PI,2*M_PI);
    TH1D* h_old_truth_missing_e = new TH1D("h_old_truth_missing_e","",1000,0,100);
    TH1D* h_old_truth_add_pid = new TH1D("h_old_truth_add_pid","",10000,-5000,5000);
    TH1D* h_old_truth_add_eta = new TH1D("h_old_truth_add_eta","",250,-1.25,1.25);
    TH1D* h_old_truth_add_dphi = new TH1D("h_old_truth_add_dphi","",640,-2*M_PI,2*M_PI);
    TH1D* h_old_truth_add_e = new TH1D("h_old_truth_add_e","",1000,0,100);
    TH1D* h_old_truth_missing_sume = new TH1D("h_old_truth_missing_sume","",1000,0,100);
    TH1D* h_old_truth_missing_n = new TH1D("h_old_truth_missing_n","",500,0,500);
    TH1D* h_old_truth_add_sume = new TH1D("h_old_truth_add_sume","",1000,0,100);
    TH1D* h_old_truth_add_n = new TH1D("h_old_truth_add_n","",500,0,500);
    TH2D* h_old_truth_new_truth = new TH2D("h_old_truth_new_truth","",1000,0,100,1000,0,100);

    TH1D* h_emcal_frac_et_transverse = new TH1D("h_emcal_frac_et_transverse","",100,-2,2);
    TH2D* h_et_transverse_emcal_frac_et_transverse = new TH2D("h_et_transverse_emcal_frac_et_transverse","",100,0,100,100,-2,2);
    ...
    //for (Long64_t entry = 0; entry < 20; ++entry) {
        ...
        int ind_truth_lead = -1; int ind_truth_sub = -1; int ind_lead = -1; int ind_sub = -1;
        float lead_e = 0; float truthlead_e = 0; float sub_e = 0; float truthsub_e = 0;
        ...
        *calibjet_pt = std::move(recocalibpt_new);
        ...
        if (bkg_cut == "dijet") {
            if (truthjet_pt->size() >= nJetReq) {
                ...
                if (truthjet_e->at(ind_truth_sub)/truthjet_e->at(ind_truth_lead) > 0.3 && match_leading_subleading_jet(truthjet_phi->at(ind_truth_lead), truthjet_phi->at(ind_truth_sub))) {
                    truth_cut = true;
                } else {
                    truth_cut = false;
                }
        ...
        */
        ...
        if (!clusters) {
            ...
            */
            ...
            for (int i = 0; i < clsmult; i++) {
                float dphi = get_dphi(lead.Phi(),cluster_phi[i]);
                if (fabs(dphi) > M_PI/3.0 && fabs(dphi) < (2.0*M_PI)/3.0 && cluster_e[i] > 0.1) { 
                    et_transverse_2sigma += cluster_e[i]/cosh(cluster_eta[i]); 
                }
            }
            // EDITED
            //for (int i = 0; i < clsmult2; i++) { // edit to use other cluster node once available 
            //    float dphi = get_dphi(lead.Phi(),cluster2_phi[i]);
            //    if (fabs(dphi) > M_PI/3.0 && fabs(dphi) < (2.0*M_PI)/3.0) { 
            //        et_transverse_ohcal_mc_data_var += cluster2_e[i]/cosh(cluster2_eta[i]); 
            //    }
            //}
                ...
                if (fabs(dphi) > M_PI/3.0 && fabs(dphi) < (2.0*M_PI)/3.0) {
                  for (int j = 0; j < cluster_ntowers[i]; j++) {
                    if (cluster_tower_calo[i][j] == 1) {
                        et_transverse_emcal += cluster_tower_e[i][j]/(cosh(cluster_eta[i]));
                    }
                  }
                }
            }
        ...
        float old_truth_et_transverse = 0;
        for (int i = 0; i < old_truthpar_n; i++) {
            if (fabs(old_truthpar_eta[i]) > 1.1) { continue; }
            if ((old_truthpar_pid[i] == 22 || old_truthpar_pid[i] == 111) && fabs(old_truthpar_e[i]) < 0.5) { continue; } // edited back to 0.2 for sPHENIX primary particle list
            else if (fabs(old_truthpar_e[i]) < 0.5) { continue; } // edited back to 0.5
            float dphi = get_dphi(truthlead.Phi(),old_truthpar_phi[i]);
            if (fabs(dphi) > M_PI/3.0 && fabs(dphi) < (2.0*M_PI)/3.0) { old_truth_et_transverse += old_truthpar_e[i]/cosh(old_truthpar_eta[i]); } 
        }
        ...
        if (reco_cut) {
            if (calibjet_pt_dijet > calibptbins[0]) {
                ...
                h_lead_spectra_record->Fill(lead.Pt(), weight_scale*vertex_weight);
                ...
                if (et_transverse != 0) { 
                    h_emcal_frac_et_transverse->Fill(et_transverse_emcal/et_transverse, weight_scale*vertex_weight);
                    h_et_transverse_emcal_frac_et_transverse->Fill(et_transverse, et_transverse_emcal/et_transverse, weight_scale*vertex_weight);
                }
            }
        }
        ...
        if (uni_truth_pt >= 0 && uni_truth_et >= 0 && uni_truth_et <= 1) {
            ...
            h_old_truth_new_truth->Fill(old_truth_et_transverse, truth_et_transverse);
            int old_truth_missing_n = 0;
            float old_truth_missing_e = 0;
            for (int i = 0; i < old_truthpar_n; i++) {
                if (fabs(old_truthpar_eta[i]) > 1.1 || fabs(old_truthpar_e[i]) < 0.5) { continue; }
                float dphi = get_dphi(truthlead.Phi(),old_truthpar_phi[i]);
                if (fabs(dphi) > M_PI/3.0 && fabs(dphi) < (2.0*M_PI)/3.0) {
                    bool match = false;
                    for (int j = 0; j < truthpar_n; j++) {
                        if (old_truthpar_e[i] == truthpar_e[j] && old_truthpar_pid[i] == truthpar_pid[j] && old_truthpar_eta[i] == truthpar_eta[j] && old_truthpar_phi[i] == truthpar_phi[j]) {
                            match = true;
                            break;
                        }
                    }
                    if (!match) {
                        h_old_truth_missing_pid->Fill(old_truthpar_pid[i]);
                        h_old_truth_missing_eta->Fill(old_truthpar_eta[i]);
                        h_old_truth_missing_dphi->Fill(dphi);
                        h_old_truth_missing_e->Fill(old_truthpar_e[i]);
                        old_truth_missing_e += old_truthpar_e[i];
                        old_truth_missing_n += 1;
                    }
                }
            }
            int old_truth_add_n = 0;
            float old_truth_add_e = 0;
            for (int i = 0; i < truthpar_n; i++) {
                if (fabs(truthpar_eta[i]) > 1.1) { continue; }
                ...
                if (fabs(dphi) > M_PI/3.0 && fabs(dphi) < (2.0*M_PI)/3.0) {
                    bool match = false;
                    for (int j = 0; j < old_truthpar_n; j++) {
                        if (truthpar_e[i] == old_truthpar_e[j] && truthpar_pid[i] == old_truthpar_pid[j] && truthpar_eta[i] == old_truthpar_eta[j] && truthpar_phi[i] == old_truthpar_phi[j]) {
                            match = true;
                            break;
                        }
                    }
                    if (!match) {
                        h_old_truth_add_pid->Fill(truthpar_pid[i]);
                        h_old_truth_add_eta->Fill(truthpar_eta[i]);
                        h_old_truth_add_dphi->Fill(dphi);
                        h_old_truth_add_e->Fill(truthpar_e[i]);
                        old_truth_add_e += truthpar_e[i];
                        old_truth_add_n += 1;
                    }
                }
            }
            h_old_truth_missing_sume->Fill(old_truth_missing_e);
            h_old_truth_missing_n->Fill(old_truth_missing_n);
            h_old_truth_add_sume->Fill(old_truth_add_e);
            h_old_truth_add_n->Fill(old_truth_add_n);
        ...
        if (iter > 1) {
            for (int i = 1; i < 3; i++) {
                ...
                if (entry % 2 == 0) { fill_trim_response_matrix(h_respmatrix_calib_dijet_half1[i+1], uni_meas_pt, calibjet_matched_dijet, uni_truth_pt, uni_meas_et, uni_truth_et, weight_scale*vertex_weight, counts_matrix[8], counts_measured[8], counts_truth[8], trim_val[i]); }
                else { fill_trim_response_matrix(h_respmatrix_calib_dijet_half2[i+1], uni_meas_pt, calibjet_matched_dijet, uni_truth_pt, uni_meas_et, uni_truth_et, weight_scale*vertex_weight, counts_matrix[9], counts_measured[9], counts_truth[9], trim_val[i]); }
        ...
        if (iter > 2) {
            for (int i = 0; i < 3; i++) {
                ...
                if (entry % 2 == 0) { fill_reweighted_trim_response_matrix(h_respmatrix_calib_dijet_half1[i+4], uni_meas_pt, calibjet_matched_dijet, uni_truth_pt, uni_meas_et, uni_truth_et, weight_scale*vertex_weight, counts_matrix[8], counts_measured[8], counts_truth[8], trim_val[i], weights[8][i]); }
                else { fill_reweighted_trim_response_matrix(h_respmatrix_calib_dijet_half2[i+4], uni_meas_pt, calibjet_matched_dijet, uni_truth_pt, uni_meas_et, uni_truth_et, weight_scale*vertex_weight, counts_matrix[9], counts_measured[9], counts_truth[9], trim_val[i], weights[9][i]); }
    ...
    h_emcal_frac_et_transverse->Write(); h_et_transverse_emcal_frac_et_transverse->Write();

    h_old_truth_missing_pid->Write();
    h_old_truth_missing_eta->Write();
    h_old_truth_missing_dphi->Write();
    h_old_truth_missing_e->Write();
    h_old_truth_add_pid->Write();
    h_old_truth_add_eta->Write();
    h_old_truth_add_dphi->Write();
    h_old_truth_add_e->Write();
    h_old_truth_missing_sume->Write();
    h_old_truth_missing_n->Write();
    h_old_truth_add_sume->Write();
    h_old_truth_add_n->Write();
    h_old_truth_new_truth->Write();
