...
cut_name = ["efrac","none"]
legend_entries = ['efrac','efrac','none','none']
...
line.Draw("SAME")
canvas.SaveAs(f"jet_bkgeff_comparison.png")
...
for i, prof in enumerate(old_profx):
    ...
    prof.GetXaxis().SetRangeUser(-1.0,15)
    prof.Draw(drawopt)
    legend.AddEntry(prof, legend_entries[i], "lep")
...
ratio.GetXaxis().SetRangeUser(-1.0,15.0)
ratio.Draw("E")
...
line.SetLineColor(ROOT.kGray + 2)
line.Draw("SAME")
canvas.SaveAs(f"etdist_bkgeff_comparison.png")
...
colors = [ROOT.kRed+1, ROOT.kBlue+1, ROOT.kGreen+2, ROOT.kMagenta+1, ROOT.kOrange+7, ROOT.kCyan+1, ROOT.kGray+1, ROOT.kBlack, ROOT.kPink+1, ROOT.kRed+3, ROOT.kBlue+3, ROOT.kGreen]
cut_name = ["efrac","none"]
legend_entries = ['efrac_new','none_new','efrac_calo_accept','none_calo_accept']
